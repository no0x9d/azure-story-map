...
<div class="min-h-screen bg-gray-50 py-12">
  <div class="max-w-lg mx-auto mt-4 px-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-6">
      ...
      <form method="POST" action="?/azure" use:enhance={makeEnhance()} class="space-y-6">
        <!-- Azure Base URL -->
        <div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
      <h2 class="text-2xl font-semibold text-gray-900 mb-1">Confluence</h2>
      <p class="text-sm text-gray-500 mb-8">
        Configure your Confluence connection for diagram export.
      </p>
      ...
      <form method="POST" action="?/confluence" use:enhance={makeEnhance()} class="space-y-6">
        <!-- Confluence Base URL -->
        <div>
          <label for="confluenceBaseUrl" class="block text-sm font-medium text-gray-700 mb-1.5">
            Confluence Base-URL
          </label>
          <input
        ...
        <!-- Confluence Email -->
        <div>
          <label for="confluenceEmail" class="block text-sm font-medium text-gray-700 mb-1.5">
            Confluence Email
          </label>
          <input
            id="confluenceEmail"
        ...
        <div>
          <label for="confluenceApiToken" class="block text-sm font-medium text-gray-700 mb-1.5">
            Confluence API Token
          </label>
          <input
            ...
            type="password"
            placeholder={data.hasConfluenceToken ? '••••••••••••••••' : 'Enter your API Token'}
            class="w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
          />
          {#if data.hasConfluenceToken}
            <p class="mt-1.5 text-xs text-gray-500">
              A token is already saved. Leave blank to keep the existing token.
            </p>
          {/if}
